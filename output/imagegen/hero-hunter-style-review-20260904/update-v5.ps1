...
$edit = Get-Content -LiteralPath "$assetDir/targeted-edits-v5-prompts.json" -Raw -Encoding UTF8 | ConvertFrom-Json
...
New-Item -ItemType Directory -Path "$assetDir/previous-v4" -Force | Out-Null
Move-Item -LiteralPath "$assetDir/final/$($edit.old)" -Destination "$assetDir/previous-v4/$($edit.old)" -Force
...
$readme += "`n최신 v5 수정: 용사 검법 캐릭터 비중을 줄이고 검날 길이와 강제 원근을 크게 과장. 상세 프롬프트는 targeted-edits-v5-prompts.json에 있습니다.`n"
...
"V5 references: $([regex]::Matches($gallery,'src="final/[^\"]+_v5.png').Count)"
$zipItems = @('before','final','index.html','review.json','manifest.json','prompts.json','targeted-edits-v3-prompts.json','targeted-edits-v4-prompts.json','targeted-edits-v5-prompts.json','README.md','qa-final-low.jpg','qa-final-high.jpg') | ForEach-Object { Join-Path $assetDir $_ }
