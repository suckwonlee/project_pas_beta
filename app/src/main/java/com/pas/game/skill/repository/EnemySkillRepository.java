...
import com.pas.game.skill.effect.Effects;
...
public final class EnemySkillRepository {
    ...
    public static SkillData unholyFlame(){return SkillData.builder("altar_unholy_flame","불경한 화염",SkillGrade.NORMAL).value(100,0).range(2).target(TargetType.ENEMY).effect(attackDamage()).effect(Effects.status(StatusType.FIRE,-1,4)).build();}
    ...
    public static SkillData hellfireFangs(){return SkillData.builder("demon_hellfire_fangs","지옥불 이빨",SkillGrade.UNCOMMON).value(70,0).range(0).cooldown(3).target(TargetType.ENEMY).effect(attackDamage()).effect(Effects.status(StatusType.FIRE,-1,3)).build();}
