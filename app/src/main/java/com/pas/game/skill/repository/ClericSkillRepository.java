...
public final class ClericSkillRepository implements SkillRepository {
    ...
    public ClericSkillRepository(){
        ...
        s.add(skill("banish_profane","불경 퇴치",SkillGrade.NORMAL,100,20,-1,0,0,0,TargetType.ENEMY,"공격력 계수 단일 피해","\"참기힘든 역겨움이네요 이 불경한 자여\"").effect(clericAttackDamage()).build());
        s.add(skill("holy_strike","거룩한 일격",SkillGrade.NORMAL,80,16,-1,0,0,0,TargetType.ENEMY,"공격력 피해와 방어력 회복","\"그녀에게 악의 정화는 봉사와도 같았고, 그녀는 봉사하며 지친적이 없었다.\"").effect(holyStrike()).build());
        s.add(skill("purifying_flame","정화의 불길",SkillGrade.NORMAL,120,25,-1,0,0,1,TargetType.ENEMY,"피해 없이 정화를 얻고 지정 대상에게 이전 조건 확인","\"타락자들이 말하는 구원이 기다리던 자리에 그녀의 기도가 울려퍼지자 정화의 불길이 피어났다.\"").effect(purifyingFlame()).build());

        s.add(skill("shield_of_faith","신앙의 방패",SkillGrade.NORMAL,300,0,-1,0,0,0,TargetType.SELF,"방어력 기반 피해 방어","\"빛이 절 보호할것입니다. 무엇도 두려울것은 없습니다.\"").effect(barrierFromDefense()).build());
        s.add(skill("healing_prayer","치유의 기도",SkillGrade.NORMAL,20,7,-1,0,0,0,TargetType.SELF,"방어력 기반 피해 감소와 잃은 HP 회복","\"추기경의 따님꼐선 정말 독실하신 분이지. 누가 부르기 전에 그분이 기도실에서 나오는걸 본사람이 없다네\"").effect(healingPrayer()).build());
        s.add(skill("divine_intervention","신성 개입",SkillGrade.NORMAL,10,0,10,2,0,0,TargetType.SELF,"최대 HP 회복과 일반 피해 무적","\"필멸자 개인의 믿음으로 저만한 빛을 끌어낼리가 없다. 천상이 그녀에게 걸어본걸지도 모르겠군\"").effect(divineIntervention()).build());
        ...
        s.add(skill("blessing_of_courage","용맹의 축복",SkillGrade.MAGIC,40,10,4,0,9,1,TargetType.ALLY,"공격력 증가","\"당신이 공포를 느낀다 해도, 당신안의 용기는 그 이상의 크기를 가지고 있습니다. 나아가십시요\"").effect(attackBlessing()).build());
        ...
        s.add(skill("answered_prayer","응답된 기도",SkillGrade.EPIC,4,0,2,1,0,1,TargetType.ALLY,"모든 스킬 쿨타임 감소","\"이계의 침공에 고통받는 이들을 위한 기도는 평소이상으로 간절했고, 그 기도는 천상에 닿았다\"").effect(answeredPrayer()).build());

        s.add(skill("crusade","성전",SkillGrade.LEGENDARY,100,50,3,0,9,1,TargetType.SELF,"범위 아군 최대 HP와 공격력·방어력 증가","\"이 땅을 구하기 위한 싸움은 매 순간이 중요하지만, 이 전투야말로 판도를 뒤집을 전투가 될것입니다.\"").effect(crusade()).build());
        ...
        s.add(skill("divine_fragment","신성의 파편",SkillGrade.OTHERWORLD,5,0,1,0,0,0,TargetType.SELF,"싱글과 멀티플레이에 따라 효과 전환","\"제가 지금 짓는것은 제 자의로 행하는 가장 큰 죄악입니다. 천상의 초대장을 이 땅의 구원에 필요한 힘으로 쓰겠나이다.\"").effect(divineFragment()).build());
        s.add(skill("divine_punishment","신벌",SkillGrade.OTHERWORLD,1,0,1,0,0,2,TargetType.ENEMY,"정화 획득을 대상의 현재·최대 HP 감소로 전환","\"그녀의 몸을 통해 뿜어저 나오는 빛에 닿자, 악의 존재가 서서히 흐려지기 시작했다.\"").effect(divinePunishment()).build());
