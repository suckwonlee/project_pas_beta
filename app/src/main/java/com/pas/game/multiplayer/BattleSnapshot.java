...
public final class BattleSnapshot {
    ...
    private static String buildJson(BattleState state,long revision){
        ...
        out.append(",\"units\":[");List<BattleUnit> units=new ArrayList<>(state.getUnits());units.sort(Comparator.comparing(BattleUnit::getUnitId));for(int i=0;i<units.size();i++){if(i>0)out.append(',');unit(out,units.get(i));}out.append(']');
    ...
    private static void unit(StringBuilder out,BattleUnit unit){
        ...
        out.append(",\"statuses\":[");List<StatusEffect> statuses=new ArrayList<>(unit.getStatuses());statuses.sort(Comparator.comparingLong(StatusEffect::getApplicationOrder).thenComparing(StatusEffect::getId));for(int i=0;i<statuses.size();i++){if(i>0)out.append(',');StatusEffect s=statuses.get(i);out.append('{');text(out,"id",s.getId());text(out,"type",s.getType().name());number(out,"remainingTurns",s.getRemainingTurns());decimal(out,"magnitude",s.getMagnitude());number(out,"stackCount",s.getStackCount());bool(out,"dispellable",s.isDispellable());out.append('}');}out.append(']');
        out.append(",\"effects\":[");for(int i=0;i<unit.getEffects().size();i++){if(i>0)out.append(',');UnitEffect e=unit.getEffects().get(i);out.append('{');text(out,"type",e.getType().name());text(out,"sourceKey",e.getSourceKey());decimal(out,"magnitude",e.getMagnitude());out.append('}');}out.append(']');
